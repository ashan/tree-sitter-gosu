...
class SHCQuoteDisplayUtil {

  var _line: entity.CWPSLine
  var _costsByCbl: Map<Coverable, List<SHCCost>>
  ...
  var _trxnsByCbl: Map<Coverable, List<SHCTransaction>>
  var _cblsWithTrxnsByType: Map<gw.lang.reflect.IType, List<Coverable>>

  construct(line: entity.CWPSLine, forCosts: Boolean) {
    _line = line
    if (forCosts) {
      initCosts()
    } else {
  ...
  private function initCosts() {
    var allCosts = _line.VersionList.SHCCosts.allVersionsFlat<SHCCost>()
  ...
  private function initTransactions() {
    ...
    _trxnsByCbl = allTrxns.toList().partition<Coverable>(\trxn -> {
      ...
      return cblKeyToCblMap.get(cost.getSlice(cost.EffDate).OwningCoverable.FixedId)
        ...
        as Map<Coverable, List<SHCTransaction>>

    _cblsWithTrxnsByType = _trxnsByCbl.Keys.toList().partition(\cbl -> (typeof cbl))
  }

  public function getCostsByCoverable(): Map<Coverable, List<SHCCost>> {
    return _costsByCbl
  }
  ...
  public function getTrxnsByCoverable(): Map<Coverable, List<SHCTransaction>> {
    return _trxnsByCbl
  }
  ...
  public function getCoverableCostsForDisplay(shcCosts : SHCCost[], levyDate : Date) : SHCCost[] {
    ...
    if (!DateUtil_ACC.isDatePriorACCEarnersResidualLevyEndDate(levyDate)) {
      var shcCosts1 = shcCosts.where(\elt -> !(elt typeis CWPSResidualEarnersLevyCost and (elt.ActualAdjRate == null or elt.ActualAdjRate == BigDecimal.ZERO)))
      return shcCosts1
    } else {
      return shcCosts
    }
  ...
  public static function CostDisplayName(cost: SHCCost): String {
    if(cost typeis CWPSWorkAccountLevyCost) {
      return cost.CWPSWorkAccountLevyCostItem.Code.concat(" - ".concat(cost.CWPSWorkAccountLevyCostItem.Description))
    } else if(cost typeis CWPSResidualWorkAccountLevyCost) {
      return cost.ResWorkAccountLevyCostItem.Code.concat(" - ".concat(cost.ResWorkAccountLevyCostItem.Description))
    } else if (cost typeis CWPSModifierCost) {
      return cost.Description
    } else if(cost typeis CWPSWorkingSaferLevyCost and DateUtil_ACC.isDatePriorACCHealthAndSafetyEndDate(cost.ExpirationDate)) {
  ...
  public static function CoverableLevyDisplayName(cbl: Coverable): String {
    switch (typeof cbl) {
      case CWPSCov:
        return "CWPS Cov: " + cbl.FixedId
      default:
        return (typeof cbl).toString() + ": " + cbl.FixedId
    }
  }
