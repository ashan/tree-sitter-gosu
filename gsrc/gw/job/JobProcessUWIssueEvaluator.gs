...
class JobProcessUWIssueEvaluator {
  ...
  private var _checkedAuditAtQuote : List<UWIssueCheckingSet> as CheckedAuditAtQuote =  {UWIssueCheckingSet.TC_PREQUOTE, UWIssueCheckingSet.TC_PREFINALAUDIT_ACC}
  ...
  function evaluateAndCheckForBlockingUWIssuesInSlices(branch : PolicyPeriod, blockingPoint : UWIssueBlockingPoint, oosSlices_ : entity.PolicyPeriod[]) {
    ...
    if (not blockingIssues.IsEmpty) {
      if(blockingIssues.hasMatch(\elt -> elt.IssueType.Code == AuditOnHoldContextDefinition_ACC.AUDIT_ON_HOLD_PREVENT_REASSESSMENT ||
          elt.IssueType.Code == ReassessmentContextDefinition_ACC.CODE_PREVENT_REASSESSMENT)) {
        ActivityUtil.createMigratedPolicyHoldActivity(branch.Job)
      }
  ...
  static function forAudit() : JobProcessUWIssueEvaluator {
    return new JobProcessUWIssueEvaluator() {
      :CheckedAuditAtQuote = {UWIssueCheckingSet.TC_PREFINALAUDIT_ACC}
    }
  }
  ...
  private function checkingSetsFor(bp : UWIssueBlockingPoint) : List<UWIssueCheckingSet> {
    if (bp == UWIssueBlockingPoint.TC_BLOCKSAUDITISSUANCE_ACC) {
      return checkingSetsForSpecificBlockingPoint(bp)
    } 
    else if (bp == UWIssueBlockingPoint.TC_BLOCKSRATERELEASE) {
  ...
  private function checkingSetsForSpecificBlockingPoint(bp : UWIssueBlockingPoint) : List<UWIssueCheckingSet> {
    switch (bp) {
      ...
      case UWIssueBlockingPoint.TC_BLOCKSAUDITISSUANCE_ACC : return CheckedAuditAtQuote
      default: return {}
    }
  }
