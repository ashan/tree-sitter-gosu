...
uses nz.co.acc.er.configuration.ERConfigurationUtil_ACC
uses nz.co.acc.er.dbconnectionhandler.StoreProcController_ACC
uses nz.co.acc.er.request.ERRetrieveExpectedSpreadsheetTabs_ACC
uses nz.co.acc.er.request.ERRetrieveFileType_ACC
uses nz.co.acc.er.request.ERRetrieveTargetfolder_ACC
uses nz.co.acc.er.request.RemoveFileImportLog_ACC
uses nz.co.acc.er.request.RetrieveFileNameforDeletion_ACC
...
class UploadParamFiles_ACC implements Serializable {
  ...
  private var _tabValidationSuccessfull: boolean = true
  private var _excelFileParser = new ExcelFileParser() as ExcelFileParser
  private var _maxFileNameLength : int = 36  // Max chars SQL server filename can be
  private var _deletedOldFile : boolean = false
...
//    }
  ...
  public function checkFileExtension()  {
    ...
    var extensions : String[] = this.FileTypeList.getExtensionsFromName(_selectedFileType).split(",")

    if ( !extensions.contains(getFileExtension().toLowerCase()) ) {
  ...
  function relocateFileToERFolder() {
    try {
      var file = new File(getErFileDestination() + truncateName(_newInputFile.NameSansExtension) + "." + _newInputFile.Extension)
  ...
  function deleteFileFromERFolder(filename : String) {
    try {
      FileUtils.forceDelete(new File(getErFileDestination() + filename))
    } catch (e : IOException) {
   ...
   * @return String Path
   */
  function getErFileDestination() : String {
  ...
  function truncateName(fullName : String) : String {
    var newName : String =  _newInputFile.NameSansExtension
