package nz.co.acc.common.function
...
class Funxion {
  ...
  public static class ExecutorBuilder<TYPE_DATA> {
    ...
    private var _when : Predicate<TYPE_DATA>

    private var _otherwise : Consumer<TYPE_DATA>

    public construct(logic: Consumer<TYPE_DATA>) {
      _logic = logic
    }

    public function when(whenLogic : Predicate<TYPE_DATA>) : ExecutorBuilder<TYPE_DATA> {
      this._when = whenLogic
      return this
    }

    public function otherwise(otherwiseLogic : Consumer<TYPE_DATA>) : ExecutorBuilder<TYPE_DATA> {
      this._otherwise = otherwiseLogic
      return this
    }

    public function execute(data : TYPE_DATA) {
      var optionalWhen = Optional.ofNullable<Predicate<TYPE_DATA>>(_when)
      var acceptLogic = \ ___data : TYPE_DATA -> _logic.accept(___data)

      if (optionalWhen.isPresent()) {
        if (optionalWhen.get().test(data)) {
          acceptLogic(data)
        } else if (Optional.ofNullable<Consumer<TYPE_DATA>>(_otherwise).isPresent()) {
          _otherwise.accept(data)
        }
      }
      else {
        acceptLogic(data)
  ...
  public static class ProcessorBuilder<TYPE_INPUT, TYPE_OUTPUT> {
    ...
    private var _when : Predicate<TYPE_INPUT>

    private var _otherwise : Function<TYPE_INPUT, TYPE_OUTPUT>

    public construct(logic : Function<TYPE_INPUT, TYPE_OUTPUT>) {
      _logic = logic
    }

    public function when(whenLogic : Predicate<TYPE_INPUT>) : ProcessorBuilder<TYPE_INPUT, TYPE_OUTPUT> {
      this._when = whenLogic
      return this
    }

    public function otherwise(otherwiseLogic : Function<TYPE_INPUT, TYPE_OUTPUT>) : ProcessorBuilder<TYPE_INPUT, TYPE_OUTPUT> {
      this._otherwise = otherwiseLogic
      return this
    }

    public function process(data : TYPE_INPUT) : TYPE_OUTPUT {
      var when = Optional.ofNullable<Predicate<TYPE_INPUT>>(_when)
      var output : TYPE_OUTPUT
      var applyLogic = \ ___data : TYPE_INPUT -> _logic.apply(___data)

      if (when.isPresent()) {
        if (when.get().test(data)) {
          output = applyLogic(data)
        } else if (Optional.ofNullable<Function<TYPE_INPUT, TYPE_OUTPUT>>(_otherwise).isPresent()) {
          output = _otherwise.apply(data)
        }
      }
      else {
        output = applyLogic(data)
   ...
   * @author Ron Webb
   */
  public static reified function buildExecutor<TYPE_DATA>(logic : Consumer<TYPE_DATA>) : ExecutorBuilder<TYPE_DATA> {
   ...
   * @author Ron Webb
   */
  public static reified function buildGenerator<TYPE_DATA>(logic : Supplier<TYPE_DATA>) : GeneratorBuilder<TYPE_DATA> {
   ...
   * @author Ron Webb
   */
  public static reified function buildProcessor<TYPE_INPUT, TYPE_OUTPUT>(logic : Function<TYPE_INPUT, TYPE_OUTPUT>) : ProcessorBuilder<TYPE_INPUT, TYPE_OUTPUT> {
