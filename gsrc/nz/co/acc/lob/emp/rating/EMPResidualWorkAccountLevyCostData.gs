...
uses entity.windowed.EMPResidualWorkAccountLevyCostVersionList
...
class EMPResidualWorkAccountLevyCostData extends EMPCostData<EMPResidualWorkAccountLevyCost> {
  ...
  construct(cost: EMPResidualWorkAccountLevyCost) {
    super(cost)
    _coveredItemID = cost.EMPWPCCov.FixedId
  }

  construct(cost: EMPResidualWorkAccountLevyCost, rateCache: PolicyPeriodFXRateCache) {
    super(cost, rateCache)
    _coveredItemID = cost.EMPWPCCov.FixedId
  }

  override function setSpecificFieldsOnCost(line: EMPWPCLine, costEntity: EMPResidualWorkAccountLevyCost): void {
  ...
  override function getVersionedCosts(line: EMPWPCLine): List<EffDatedVersionList> {
    ...
    return coveredItemVL.EMPResidualWorkAccountLevyCosts.where(\costVL -> isCostVersionListForThisCostData(costVL))
  }

  private function isCostVersionListForThisCostData(costVL: EMPResidualWorkAccountLevyCostVersionList): boolean {
