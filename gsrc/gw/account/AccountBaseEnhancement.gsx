...
enhancement AccountBaseEnhancement : Account {
  ...
  property get HasPolicy_ACC() : Boolean {
    return Query.make(Policy)
        .compare(Policy#Account, Equals, this)
        .select()
  ...
  static function createAccountForContact(contact : Contact) : Account {
    ...
    //ACC - need to populate the ACC account number when creating from contact if account number exists.
    if (!GosuStringUtil.isEmpty(contact.ACCID_ACC)) {
      account.ACCID_ACC = contact.ACCID_ACC
    }
