...
uses nz.co.acc.scriptparameter.ScriptParametersEnhancement_ACC
...
class RenewalProcess extends NewTermProcess implements IRenewalProcess {
  ...
  // Number of days until when a quoted renewal job shall remain editable
  private final static var RENEWALS_EDITABILITY_ACC = (ScriptParameters.getParameterValue("RenewalsEditabilityWindow") as BigDecimal).intValue()
  // Maximum number of days allowed until when a PRD can be created before a renewal job is quoted
  private final static var MAX_ALLOWED_DAYS_FOR_PRD_ACC = ScriptParameters.MaxAllowedDaysForPRD_ACC

  protected static final var _BATCH_RENEWAL_USER :String as readonly BATCH_RENEWAL_USER = "sys"

  private var PENDING_RENEWAL_FIRST_CHECK_DATE_OFFSET_ACC = -80
  ...
  override function start() {
    ...
    // Same for IR, this stops errors appearing in the logs.  TODO - this may not solve root cause of problem.
    if (Job.TriggerReason_ACC != ReasonCode.TC_IR_ACC) {
      doStart()
    }
  }
  ...
  function doStart() {
    ...
    if (Job.ActivePeriods.Count == 1) {
      ...
      // DE689 - Manual renewal jobs shall commence in EDIT mode by default
      //       - No change intended to how the automatic renewals are handled
      //       - The assumption here is that automatic renewal batch jobs will always be run using SYS user
      if (User.util.CurrentUser.Credential.UserName.equalsIgnoreCase(BATCH_RENEWAL_USER)) {
  ...
  function escalate(subject : String, description : String) {
    ...
    // DE1531 - Assign Renewal Escalations to the policy admin queue
    var pattern = ActivityPattern.finder.getActivityPatternByCode("open_eval_issue")
    var activity = pattern.createJobActivity(Job.Bundle, Job, subject, description, null, null, null, null, null)
    var renewalEscalationActivityQueue = AssignableQueueUtils.getQueueForRenewalEscalation()
    if (renewalEscalationActivityQueue != null) {
      activity.assignActivityToQueue(renewalEscalationActivityQueue, renewalEscalationActivityQueue.getGroup())
    } else {
      JobProcessLogger.logError("Renewal Activity Queue not found for branch: " + _branch)
    }
  ...
  protected function startPendingRenewal() {
    ...
    if (escalationReasonChecker.ShouldEscalate) {
      ...
      try {
        ...
        JobProcessLogger.logError("Exception in startPendingRenewal for branch: " + _branch, e)
        var reasonChecker = new EscalationReasonChecker(TC_RENEWING)
  ...
  protected property get PendingRenewalFirstCheckDate() : Date {
    return _branch.PeriodStart.addDays(PENDING_RENEWAL_FIRST_CHECK_DATE_OFFSET_ACC)
  }

  /**
   * Enables overriding the hardcoded OOTB value for PendingRenewalFirstCheckDate. Useful for testing pending renewals.
   */
  public function overridePendingRenewalFirstCheckDateOffset_ACC(pendingRenewalFirstCheckDateDateOffset : Integer) {
    this.PENDING_RENEWAL_FIRST_CHECK_DATE_OFFSET_ACC = pendingRenewalFirstCheckDateDateOffset
  }
  ...
  function pendingRenewalFinalCheck() {
    ...
    if (escalationReasonChecker.ShouldEscalate) {
      ...
      // We do not need to send renewal documents as part of the pre renewal final check
  ...
  protected property get IssueAutomatedRenewalDate() : Date {
    return _branch.PeriodStart.addDays(RENEWALS_EDITABILITY_ACC)
  }
  ...
  function issueAutomatedRenewal() {
    try {
      ...
      JobProcessLogger.logError("Exception in issueAutomatedRenewal for branch: " + _branch, e)
      var reasonChecker = new EscalationReasonChecker(TC_RENEWING)
                              .addCustomError(DisplayKey.get("Job.Renewal.Escalation.Reason.ValidationErrors"))
      escalate(reasonChecker.ActivitySubject, reasonChecker.ActivityDescription)
    } catch (e : UWAuthorityBlocksProgressException) {
      JobProcessLogger.logWarning("Exception in issueAutomatedRenewal for branch: " + _branch, e)
      var reasonChecker = new EscalationReasonChecker(TC_RENEWING)
                              .addCustomError(DisplayKey.get("Job.Renewal.Escalation.Reason.UWIssues"))
      escalate(reasonChecker.ActivitySubject, reasonChecker.ActivityDescription)
    } catch (e : Throwable) {
  ...
  public function preSchedulePendingRenewal() {
  ...
  function pendingNonRenewalFinalCheck() {
    ...
    if (escalationReasonChecker.ShouldEscalate) {
      ...
      // We do not need to send non renewal documents as part of the non renewal final check
  ...
  protected property get SendNonRenewalDate() : Date {
    return _branch.PeriodStart.addDays(MAX_ALLOWED_DAYS_FOR_PRD_ACC)
  }
  ...
  override function canNonRenew() : boolean {
    ...
    try {
      ...
      PCLoggerCategory.JOB_PROCESS.error("Exception encountered in canNonRenew", e)
      return false
    }
  }
  ...
  protected function hasValidationIssues() : boolean {
    try {
      ...
      PCLoggerCategory.JOB_PROCESS.error("Exception encoutered in hasValidationIssues", e)
      return true
    }
