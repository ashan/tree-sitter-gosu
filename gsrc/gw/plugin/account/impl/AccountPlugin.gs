...
@Export
class AccountPlugin extends AbstractAccountPlugin {
  ...
  override function populateAccountSummary(summary : AccountSummary, account : Account) : void {
    summary.setACCID_ACC(account.getACCID_ACC())
    summary.setAEPContractNumber_ACC(account.getAEPContractNumber_ACC())
  }
  ...
  override function generateAccountNumber(account : Account) : String {
    ...
    var uniquelyGenerated = false
    var result : String

    while (not uniquelyGenerated) {
      ...
      var numberOfAccounts = Query.make(Account)
          .compare(Account#AccountNumber, Relop.Equals, accountNumber.toString())
          .select()
          .getCountLimitedBy(1)

      if (numberOfAccounts < 1) {
        result = accountNumber.toString()
        uniquelyGenerated = true
      }
    }
    return result
  }
  ...
  override function getInverseRelationshipType(relationshipType : AccountRelationshipType) : AccountRelationshipType {
    switch (relationshipType) {
      ...
      case TC_AEP_PRIME_ACC:
        return TC_AEP_PRIME_OF_ACC
      case TC_AEP_PRIME_OF_ACC:
        return TC_AEP_PRIME_ACC
      default:
