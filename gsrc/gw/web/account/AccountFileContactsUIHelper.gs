...
uses nz.co.acc.validation.AccountContactRolesValidation_ACC
uses typekey.*
...
class AccountFileContactsUIHelper {
  ...
  public static function toogleHideContact_ACC(accountContact : AccountContact) {
    if (canHideContact(accountContact)) {
      gw.transaction.Transaction.runWithNewBundle(\bundle -> {
        accountContact = bundle.add(accountContact)
        accountContact.setHidden_ACC(!accountContact.Hidden_ACC)
      })
    } else {
      throw new DisplayableException("Cannot hide primary contact/account holder.")
    }
  }


  public static function canHideContact(accountContact : AccountContact) : boolean {
    return perm.System.hidecontact_acc and !accountContact.Primary_ACC and accountContact.Account.AccountHolderContact != accountContact.Contact
  }
  ...
  public static function addContactFromAddressBook(acctContactRolesDisplayNames : java.util.Map, account : entity.Account, abContact : Contact, roleType : typekey.AccountContactRole) {
    ...
    //ACC - Add warning if the shareholder contact does not have an Account Number
    if (roleType == TC_SHAREHOLDERCONTACT_ACC and GosuStringUtil.isEmpty(abContact.ACCID_ACC)) {
      gw.api.util.LocationUtil.addRequestScopedWarningMessage(DisplayKey.get("Web.AccountContactsLV.Warning.Shareholder.Contact.Missing.AccountNumber_ACC"))
    }

    // ACC - Do role validation for required role data
    AccountContactRolesValidation_ACC.validateRoleData(account.AccountContacts)
    // reload
