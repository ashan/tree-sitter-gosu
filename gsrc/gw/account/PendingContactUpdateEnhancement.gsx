...
enhancement PendingContactUpdateEnhancement : entity.PendingContactUpdate {
  ...
  function applyUpdateToAccountEntity(){
    ...
    if (target typeis Person){
      ...
      if (update.Prefix != null or update.PrefixIsNull){
        target.Prefix = update.Prefix
      }
      if (update.MiddleName != null or update.MiddleNameIsNull){
        target.MiddleName = update.MiddleName
      }
