...
class AffinityGroupValidation extends PCValidationBase {
  ...
  function validateAffinityGroupProducerCodes() {
    ...
    if (AffinityGroup.AffinityGroupType == AffinityGroupType.TC_OPEN and not AffinityGroup.AffinityGroupProducerCodes.IsEmpty) {
      Result.addError(AffinityGroup, ValidationLevel.TC_DEFAULT, DisplayKey.get("Web.Admin.AffinityGroupProducerCodesNotAllowed_ACC"))
    }
  }
  ...
  function validateOrganizationOnAffinityGroup() {
    ...
    if (AffinityGroup.AffinityGroupType == AffinityGroupType.TC_OPEN and AffinityGroup.Organization != null) {
      Result.addError(AffinityGroup, ValidationLevel.TC_DEFAULT, DisplayKey.get("Web.Admin.AffinityGroupOrganizationNotAllowed_ACC"))
    }
  }
  ...
  function validateAffinityGroupDates() {
    ...
    if (startDate != null and endDate != null){
      if(startDate.compareTo(endDate) == 0) {
        Result.addWarning(AffinityGroup, ValidationLevel.TC_DEFAULT, DisplayKey.get("Web.Admin.AffinityGroupSameStartAndEndDates_ACC"))
      } else if (startDate.compareTo(endDate) > 0 ) {
        Result.addError(AffinityGroup, ValidationLevel.TC_DEFAULT, DisplayKey.get("Web.Admin.AffinityGroupEndDateBeforeStartDate_ACC"))
      }
    }
  }
}
