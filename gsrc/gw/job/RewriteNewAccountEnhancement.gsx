  ...
  static function startRewriteNewAccount(policyPeriods : PolicyPeriod[], targetAccount : Account) {
    ...
    for (policyPeriod in policyPeriods) {
      Transaction.runWithNewBundle(\bundle -> {
        var sourcePolicy = bundle.add(policyPeriod.Policy)
        var sourceTerm = bundle.add(policyPeriod.PolicyTerm)

        // Reset VFC
        sourceTerm.VFCUpdatePending_ACC = true
        sourceTerm.VFCOverrideDate_ACC = null
        ...
        // DE810 and DE1181 - Do not create activity for AEP
        if (!targetAccount.AEPContractAccount_ACC and !sourcePolicy?.Account?.AEPContractAccount_ACC) {
