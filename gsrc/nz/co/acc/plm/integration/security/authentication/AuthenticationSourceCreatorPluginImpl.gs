...
class AuthenticationSourceCreatorPluginImpl implements AuthenticationSourceCreatorPlugin {
  ...
  override function createSourceFromHTTPRequest(request: HttpServletRequest): AuthenticationSource {
    ...
    if (code != null) {
      ...
      if (KeyVaultAzureAPI.isEnabled() and userName != null) {
        return new KeyVaultAuthenticationSource(userName, password)
      } else {
