...
uses nz.co.acc.gwer.bulkupload.row.ERInjuryCodeRow
 ...
 */
class ERInjuryCodeProcessor extends AbstractXLSSheetProcessor {
  public static var SHEET_NAME : String = "er_injury_codes"
  construct(updater : BulkUploadProcessUpdater, sheet : Sheet) {
    super(updater, sheet)
    _log = StructuredLogger.CONFIG.withClass(ERInjuryCodeProcessor)
  }

  public static function newInstance(
      updater : BulkUploadProcessUpdater,
      sheet : Sheet) : ERInjuryCodeProcessor {
    var parser = new ERParameterValueParser()
    return new ERInjuryCodeProcessor(updater, sheet)
  }

  function processRows(parsedRows : List<ERInjuryCodeRow>) : XLSProcessorResult {
    return null
  }
...
//              }
//            } catch (e : Exception) {
//              _log.error_ACC("processSheet parameter exists", e)
//              onFailure()
//            }
//          }
//          lineNumber += 1
...
//  }
  ...
  override function processRow(list : List<Cell>) {
    var erRow = new ERInjuryCodeRow()
    erRow.injuryCode = list.get(0).StringCellValue
    erRow.codingSystem = list.get(1).StringCellValue
    erRow.injuryDescription = list.get(2).StringCellValue
    erRow.injuryCategory = list.get(3).StringCellValue
    ...
    gw.transaction.Transaction.runWithNewBundleAsSystemUser_ACC(\b -> {
      ...
      if(param == null) {
        param = new ERParamInjuryCode_ACC()
      } else {
        param = b.add(param)
      }
      param.InjuryCode = erRow.injuryCode
      param.CodingSystem = erRow.codingSystem
      param.InjuryDesc = erRow.injuryDescription
      param.InjuryCategory = erRow.injuryCategory
    })
    _log.info("processSheet parameter added ${erRow.toString()}")
  }

  function getRecord(row : ERInjuryCodeRow) : ERParamInjuryCode_ACC {
    var erParamQuery = Query.make(ERParamInjuryCode_ACC)
        erParamQuery.compare(ERParamInjuryCode_ACC#InjuryCode, Relop.Equals, row.injuryCode)
