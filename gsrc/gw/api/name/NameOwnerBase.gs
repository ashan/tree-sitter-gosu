...
abstract class NameOwnerBase implements NameOwner {
  ...
  var _personMandatoryFields = {"LastName"}
  var _companyMandatoryFields = {"Name"}
  ...
  override function isRequired(fieldId : NameOwnerFieldId) : boolean {
    if(ContactName.Migrated_ACC){
      if(ContactName typeis PersonNameFields) {
        return _personMandatoryFields.hasMatch(\elt1 -> elt1.equals(fieldId.Name))
      } else {
        return _companyMandatoryFields.hasMatch(\elt1 -> elt1.equals(fieldId.Name))
      }
    } else {
