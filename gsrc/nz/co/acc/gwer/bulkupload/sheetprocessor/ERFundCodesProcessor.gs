...
uses nz.co.acc.gwer.bulkupload.row.ERFundCodesRow
 ...
 */
class ERFundCodesProcessor extends AbstractXLSSheetProcessor {
  public static var SHEET_NAME : String = "er_fund_codes"
  construct(updater : BulkUploadProcessUpdater, sheet : Sheet) {
    super(updater, sheet)
    _log = StructuredLogger.CONFIG.withClass(ERFundCodesProcessor)
  }

  public static function newInstance(
      updater : BulkUploadProcessUpdater,
      sheet : Sheet) : ERFundCodesProcessor {
    var parser = new ERParameterValueParser()
    return new ERFundCodesProcessor(updater, sheet)
  }

  function processRows(parsedRows : List<ERFundCodesRow>) : XLSProcessorResult {
    return null
  }
...
//  }
  ...
  override function processRow(list : List<Cell>) {
    var erRow = new ERFundCodesRow()
    erRow.levyApplicationYear = Double.valueOf(list.get(0).NumericCellValue).intValue()
    erRow.fundCode = String.valueOf(list.get(1).NumericCellValue)
    erRow.fundName = list.get(2).StringCellValue
    erRow.experienceRatingInd = list.get(3).StringCellValue.equalsIgnoreCase("y")
    ...
    gw.transaction.Transaction.runWithNewBundleAsSystemUser_ACC(\b -> {
      ...
      if(param == null) {
        param = new ERParamFundCode_ACC()
      } else {
        param = b.add(param)
      }
      param.LevyApplicationYear = erRow.levyApplicationYear
      param.FundCode = erRow.fundCode
      param.FundName = erRow.fundName
      param.ExperienceRatingInd = erRow.experienceRatingInd
    })
    _log.info("processSheet parameter added ${erRow.toString()}")
  }

  function getRecord(row : ERFundCodesRow) : ERParamFundCode_ACC {
    var erParamQuery = Query.make(ERParamFundCode_ACC)
        erParamQuery.compare(ERParamFundCode_ACC#LevyApplicationYear, Relop.Equals, row.levyApplicationYear)
        erParamQuery.compare(ERParamFundCode_ACC#FundCode, Relop.Equals, row.fundCode)
