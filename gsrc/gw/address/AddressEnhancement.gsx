...
enhancement AddressEnhancement : Address {
   ...
   * Find all contacts which have an address (primary or secondary) linked to this address.
   */
  function findLinkedContacts_ACC() : List<Contact> {
    if (this.New) {
      return {}
    }

    var primaryQuery = Query.make(Contact)
        .compare(Contact#PrimaryAddress, Relop.Equals, this)

    var secondaryQuery = Query.make(Contact)
        .join(ContactAddress, "Contact")
        .join("Address")
        .compare(Address#ID, Relop.Equals, this.ID)
  ...
  function mergeLinkedAddresses(victim : Address) {
    ...
    victimLinkedAddress.Addresses.each(\a -> {
      a.LinkedAddress = survivorLinkedAddress
    })
  ...
  function clearAddressFlags() {
    this.IsWPCAddress_ACC = false
    this.IsWPSAddress_ACC = false
    this.IsCPCPXAddress_ACC = false
  }
