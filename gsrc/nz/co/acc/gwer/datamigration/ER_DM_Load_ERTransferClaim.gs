...
class ER_DM_Load_ERTransferClaim {

  public function MigrateERData(csvPath : String) {
    var executor = Executors.newWorkStealingPool(40)
    var inputFile = new File(csvPath+"ERTransferClaim.csv")
    var bulkUploader : ERTransferClaimBulkUploader
    gw.transaction.Transaction.runWithNewBundleAsSystemUser_ACC(\bundle -> {
      bulkUploader = new ERTransferClaimBulkUploader(inputFile, inputFile.Name, BulkUploadType_ACC.TC_ERDATAMIGRATION)
    })
    try {
  ...
  class ERTransferClaimBulkUploader extends AbstractBulkUploader {
    ...
    override protected function getCSVProcessor(bulkUploadType : BulkUploadType_ACC, updater : BulkUploadProcessUpdater, uploadFile : File) : AbstractCSVProcessor {
      return new BulkERTransferClaimProcessor(new ERTransferClaimUploadParser(), updater, uploadFile)
    }
  }

  /*--- ER Data Objects ---*/
  class ERTransferClaimUploadDTO {
    ...
    public override function toString(): String {
      return "ERTransferClaimUploadDTO{" +
          "erID =" + erID + '' +
  ...
  /*--- ER Upload Parsers ---*/
  class ERTransferClaimUploadParser implements IRowParser<ERTransferClaimUploadDTO> {
    private final var dateParser = new DateParser()
    override function parseRow(csvParser : CSVParser) : Either<List<FieldValidationError>, ERTransferClaimUploadDTO> {
      try {
        ...
        var parseErrors = verifyPresenceOfMandatoryFields(transferID, aCCPolicyID, claimNumber)
        var dto = new ERTransferClaimUploadDTO()
    ...
    private function verifyPresenceOfMandatoryFields(
        ...
        claimNumber : Optional<String>) : List<FieldValidationError> {
  ...
  class BulkERTransferClaimProcessor extends AbstractCSVProcessor<ERTransferClaimUploadDTO> {
    construct(rowParser : IRowParser<ERTransferClaimUploadDTO>, updater : BulkUploadProcessUpdater, uploadFile : File) {
      super(rowParser, updater, uploadFile)
      _log = StructuredLogger.CONFIG.withClass(BulkERTransferClaimProcessor)
    }

    override function processRows(rows : List<ERTransferClaimUploadDTO>) : CSVProcessorResult {
      var dataRows = rows
      var rowProcessErrors = new ArrayList<RowProcessError>()
      _log.info("Importing ${dataRows.Count} ERTransferClaims...")

      for (dataRow in dataRows index i) {
        var rowNumber = i + 1
        var lineNumber = i + 2
        createERTransferClaim(lineNumber, dataRow, rowProcessErrors)
      }
      return new CSVProcessorResult(dataRows.Count - rowProcessErrors.Count, rowProcessErrors)
    }

    private function createERTransferClaim(lineNumber : int, dto : ERTransferClaimUploadDTO, rowProcessErrors : List<RowProcessError>) {
      try {
        var oEntity : ERTransferClaim_ACC
        gw.transaction.Transaction.runWithNewBundle(\bundle -> {
          oEntity = new ERTransferClaim_ACC()
          ...
          _log.info("${lineNumber}: Created ER Transfer Claim")
        }, "sys")
        onSuccess()
      } catch (e : Exception) {
        _log.info("${lineNumber}: Creation of ER Transfer Claim failed for ${dto}: ${e.Message}")
