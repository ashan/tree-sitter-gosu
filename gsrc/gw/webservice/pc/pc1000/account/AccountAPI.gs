...
class AccountAPI {
  ...
  //ACC added code
  /**
   * Instructs PC to update the account status.
   *
   * @param accountNumber account number of the Account
   * @return boolean
   */
  ...
  @Throws(BadIdentifierException, "If cannot find entity with given identifier")
  @Throws(IllegalArgumentException, "If an argument is illegal")
  @Param("account ID", "public ID of the Account")
  function evaluateAccountStatus(accountId : String, balance : BigDecimal) : void {
    SOAPUtil.require(accountId, "AccountID" )
    var accountStatusEvaluator = new AccountStatusEvaluator()
    var account : Account = findRequiredAccount(accountId)
    Transaction.runWithNewBundle(\bundle -> {
      bundle.add(account)
      accountStatusEvaluator.evaluateNewAccountStatus(account, Optional.of<BigDecimal>(balance))
    })
  }
}
