...
class PolicyTermPlugin implements IPolicyTermPlugin {
  ...
  private function calculatePeriodEnd(effDate : Date, term : TermType, policyPeriod : PolicyPeriod,
    ...
    switch (term) {
      ...
      case TermType.TC_OTHER:
        // US1251 - Other defaults to the next ACC levy year start
        futureDate = DateUtil_ACC.nextACCLevyYearStart(effDate)
        break
      default:
  ...
  private function reconcileEndDate(effDate : Date, futureDate : Date, basedOnpolicyPeriod : PolicyPeriod, term : TermType) : Date {
    // Defect DE-31 - Do not reconcile the end date for term type OTHER
    if (TermType.TC_OTHER == term) {
      return futureDate
    }
