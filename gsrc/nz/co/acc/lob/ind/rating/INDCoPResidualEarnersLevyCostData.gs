...
uses entity.windowed.INDCoPResidualEarnersLevyCostVersionList
...
class INDCoPResidualEarnersLevyCostData extends INDCostData<INDCoPResidualEarnersLevyCost> {
  ...
  construct(cost: INDCoPResidualEarnersLevyCost) {
    super(cost)
    _coveredItemID = cost.INDCoPCov.FixedId
  }

  construct(cost: INDCoPResidualEarnersLevyCost, rateCache: PolicyPeriodFXRateCache) {
    super(cost, rateCache)
    _coveredItemID = cost.INDCoPCov.FixedId
  }

  override function setSpecificFieldsOnCost(line: INDCoPLine, costEntity: INDCoPResidualEarnersLevyCost): void {
  ...
  override function getVersionedCosts(line: INDCoPLine): List<EffDatedVersionList> {
    ...
    return coveredItemVL.INDCoPResidualEarnersLevyCosts.where(\costVL -> isCostVersionListForThisCostData(costVL))
  }

  private function isCostVersionListForThisCostData(costVL: INDCoPResidualEarnersLevyCostVersionList): boolean {
