...
uses entity.windowed.INDCoPWorkingSaferLevyCostVersionList
...
class INDCoPClassificationUnitCostData extends INDCostData<INDCoPWorkingSaferLevyCost> {
  ...
  construct(cost: INDCoPWorkingSaferLevyCost) {
    super(cost)
    _coveredItemID = cost.INDCoPCov.FixedId
  }

  construct(cost: INDCoPWorkingSaferLevyCost, rateCache: PolicyPeriodFXRateCache) {
    super(cost, rateCache)
    _coveredItemID = cost.INDCoPCov.FixedId
  }

  override function setSpecificFieldsOnCost(line: INDCoPLine, costEntity: INDCoPWorkingSaferLevyCost): void {
  ...
  override function getVersionedCosts(line: INDCoPLine): List<EffDatedVersionList> {
    ...
    return coveredItemVL.INDCoPWorkingSaferLevyCosts.where(\costVL -> isCostVersionListForThisCostData(costVL))
  }

  private function isCostVersionListForThisCostData(costVL: INDCoPWorkingSaferLevyCostVersionList): boolean {
