...
class AddressFormatter extends AddressFillableExtensionImpl {
  ...
  private function internalFormat(address : AddressFillable, delimiter : String) : String {
    ...
    if (_addrCountry == TC_JP) {
      ...
      // common case
      append(sb, fieldId.ATTENTION_ACC, delimiter, addr.Attention_ACC)
      if(isNotUnknown(addr.AddressLine1)) {
        append(sb, fieldId.ADDRESSLINE1, delimiter, addr.AddressLine1)
      }
      if(isNotUnknown(addr.AddressLine2)) {
        append(sb, fieldId.ADDRESSLINE2, delimiter, addr.AddressLine2)
      }

      if(isNotUnknown(addr.AddressLine3)) {
      ...
      switch(_addrCountry) {
        ...
        default:
          ...
          if(isNotUnknown(addr.PostalCode)) {
            append(cszBuf, fieldId.POSTALCODE, " ", addr.PostalCode)
          }
...
//    if (_includeCountry and _addrCountry != defaultCountry) {
    ...
    if(isNotUnknown(countryValue)) {
      append(sb, fieldId.COUNTRY, delimiter, countryValue)
    }

//    }

    if (_includeCounty and _addrCountry == TC_NZ and isNotUnknown(addr.County)) {
  ...
  function isNotUnknown(fieldValue : String) : boolean {
    if(fieldValue != null and !fieldValue.equalsIgnoreCase("unknown")) {
      return true
    }
    return false
  }
