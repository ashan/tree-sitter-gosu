...
class PolicyChangeProcess extends JobProcess implements IPolicyChangeProcess {
  ...
  override function bind() {
    ...
    try {
      ...
      PCLoggerCategory.JOB_PROCESS.error("Unable to issue Policy Change", e)
  ...
  override property get BillingSubjectToFinalAudit() : boolean {
    return _branch.Canceled
        and _branch.CancelReasonCode != ReasonCode.TC_JOINEDAEPGROUP_ACC
        and (_branch.hasScheduledFinalAudit() or _branch.hasOpenFinalAudit())
  }
  ...
  override function issueJob(bindAndIssue : boolean) {
    ...
    bind()
    if(_branch.BasedOn.NewAEPCustomer_ACC != _branch.NewAEPCustomer_ACC) {
      _branch.onNewAEPCustomerFlagChanged_ACC(_branch.NewAEPCustomer_ACC)
    }

    if(_branch.BasedOn.CeasedTrading_ACC != _branch.CeasedTrading_ACC) {
      _branch.onCeaseFlagChange_ACC(_branch.LevyYear_ACC)
      // Setting the Ceased Trading Date value back to PolicyTerm entity
      _branch.PolicyTerm.CeasedTradingDate_ACC = _branch.CeasedTradingDate_ACC
    }

    // Check whether levy year - 2  is ceased
    // Only uncease for IND CoP Line
    if (_branch.INDCoPLineExists) {

      var previousYear = _branch.LevyYear_ACC - 2
      if(_branch.IsNewLERuleAppliedYear) {
        previousYear = _branch.LevyYear_ACC - 1
      }

      var noUnceaseBeforeAndIncludingLevyYear_ACC = (ScriptParameters.getParameterValue("NoUnceaseBeforeAndIncludingLevyYear_ACC") as BigDecimal).intValue()
      if(_branch.IsLETransitionYear) {
        if(_branch.isLevyYearCeased(previousYear) and _branch.INDCoPLine.totalLENotZeroAndChanged(true)
            and _branch.LevyYear_ACC > noUnceaseBeforeAndIncludingLevyYear_ACC) {
          _branch.unceasePolicyOnLevyYear_ACC(previousYear)
        } else if(_branch.isLevyYearCeased(previousYear-1) and _branch.INDCoPLine.totalLENotZeroAndChanged(false)
            and _branch.LevyYear_ACC > noUnceaseBeforeAndIncludingLevyYear_ACC) {
          _branch.unceasePolicyOnLevyYear_ACC(previousYear-1)
        }
      } else {
        if(_branch.isLevyYearCeased(previousYear) and _branch.INDCoPLine.totalLiableEarningsNotZeroAndChanged()
            and _branch.LevyYear_ACC > noUnceaseBeforeAndIncludingLevyYear_ACC) {
          _branch.unceasePolicyOnLevyYear_ACC(previousYear)
        }
      }
    }
    // If CPX line has been removed then remove CPX from any future renewing policy periods
    if (_branch.BasedOn.INDCPXLineExists and not _branch.INDCPXLineExists) {
      _branch.cancelFutureRenewingCPXPolicyPeriods()
    }
  }
  ...
  function canPostToBC_ACC() : boolean {
    if (not _branch.IsAEPMasterPolicy_ACC)
      return false

    if (_branch.Status != PolicyPeriodStatus.TC_DRAFT)
      return false

    if (_branch.AEPLineExists and _branch.AEPLine.AEPMemberData.HasElements)
      return false

    if (not _branch.PolicyTerm.canPerformAEPAction_ACC(AEPAction_ACC.TC_SHOW_POST_TO_BC_BUTTON))
      return false
