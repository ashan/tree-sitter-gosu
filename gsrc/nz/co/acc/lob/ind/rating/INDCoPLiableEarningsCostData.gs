...
class INDCoPLiableEarningsCostData extends INDCostData<INDCoPLiableEarningsCost> {
  ...
  construct(cost: INDCoPLiableEarningsCost) {
    super(cost)
    _coveredItemID = cost.INDCoPCov.FixedId
  }

  construct(cost: INDCoPLiableEarningsCost, rateCache: PolicyPeriodFXRateCache) {
    super(cost, rateCache)
    _coveredItemID = cost.INDCoPCov.FixedId
  }

  override function setSpecificFieldsOnCost(line: INDCoPLine, costEntity: INDCoPLiableEarningsCost): void {
  ...
  override function getVersionedCosts(line: INDCoPLine): List<EffDatedVersionList> {
    ...
    return coveredItemVL.INDCoPLiableEarningsCosts.where(\costVL -> isCostVersionListForThisCostData(costVL))
  }

  private function isCostVersionListForThisCostData(costVL: INDCoPLiableEarningsCostVersionList): boolean {
