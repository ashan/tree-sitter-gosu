...
internal final class TypeAnalyzer {
  ...
  private static var LOGGER =  StructuredLogger.EDGE_SP.withClass(TypeAnalyzer)
  ...
  private function extractMetadata(t : IType) : TypeMetadata {
    ...
    if (!isAccessible(t)) {
      if (t.TypeInfo.Constructors.hasMatch(\ ctor -> ctorHasDIAnnotations(ctor))) {
        LOGGER.warn_ACC(t.QName + ": Type is not accessible but has DI annotations on it")
      }
      return null
    }
    ...
    for (var ref in refs) {
      if (!allParamNames.contains(ref.Name)) {
        LOGGER.warn_ACC(t.QName + ": Reference specification for unused parameter " + ref.Name)
      }
    }

    for (var doc in docs) {
      if (!allParamNames.contains(GosuDoc.getParamName(doc))) {
        LOGGER.warn_ACC(t.QName + ": Param specification for unused parameter " + GosuDoc.getParamName(doc))
      }
    }
  ...
  private function analyzeParameter(param : IParameterInfo, refs : List<NodeParamRef>, docs : List<Param>, parent : IType) : CtorParamMetadata {
    ...
    if (doc.length > 1) {
      LOGGER.warn_ACC(parent.Namespace + "." + parent.Name + ": Constructor parameter " + param.Name + " has more than one documentation annotation")
  ...
  private function findUniqueInjectableCtor(t : IType) : IConstructorInfo {
    ...
    for (var ctor in annotatedCtors) {
      if (!ctor.Public) {
        LOGGER.warn_ACC(t.Namespace + "." + t.Name + ": Type has a DI-annotated inaccessible ctor " + ctor)
      }
    }
    ...
    if (accessibleCtors.length != 1) {
      LOGGER.error_ACC(t.Namespace + "." + t.Name + ": Type has no unique accessible DI-annotated constructor")
      return null
    }
