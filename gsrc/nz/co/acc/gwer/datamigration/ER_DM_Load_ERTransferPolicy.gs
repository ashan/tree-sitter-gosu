...
class ER_DM_Load_ERTransferPolicy {

  public function MigrateERData(csvPath : String) {
    var executor = Executors.newWorkStealingPool(40)
    var inputFile = new File(csvPath+"ERTransferPolicy.csv")
    var bulkUploader : ERTransferPolicyBulkUploader
    gw.transaction.Transaction.runWithNewBundleAsSystemUser_ACC(\bundle -> {
      bulkUploader = new ERTransferPolicyBulkUploader(inputFile, inputFile.Name, BulkUploadType_ACC.TC_ERDATAMIGRATION)
    })
    try {
  ...
  class ERTransferPolicyBulkUploader extends AbstractBulkUploader {
    ...
    override protected function getCSVProcessor(bulkUploadType : BulkUploadType_ACC, updater : BulkUploadProcessUpdater, uploadFile : File) : AbstractCSVProcessor {
      return new BulkERTransferPolicyProcessor(new ERTransferPolicyUploadParser(), updater, uploadFile)
    }
  }

  /*--- ER Data Objects ---*/
  class ERTransferPolicyUploadDTO {
    ...
    public var liableEarningsAmount: MonetaryAmount as LiableEarningsAmount = null
    ...
    public override function toString(): String {
      return "ERTransferPolicyUploadDTO{" +
          "erID =" + erID + '' +
          ", transferID ='" + transferID + '\'' +
          ", aCCPolicyID ='" + aCCPolicyID + '\'' +
          ", levyYear ='" + levyYear + '\'' +
          ", cuCode ='" + cuCode + '\'' +
          ", liableEarningsAmount ='" + liableEarningsAmount + '\'' +
  ...
  /*--- ER Upload Parsers ---*/
  class ERTransferPolicyUploadParser implements IRowParser<ERTransferPolicyUploadDTO> {
    private final var dateParser = new DateParser()
    override function parseRow(csvParser : CSVParser) : Either<List<FieldValidationError>, ERTransferPolicyUploadDTO> {
      try {
        ...
        var liableEarningsAmount = csvParser.nextString().trim().toOptional()
        ...
        var parseErrors = verifyPresenceOfMandatoryFields(transferID, aCCPolicyID, levyYear, cuCode, liableEarningsAmount)
        var dto = new ERTransferPolicyUploadDTO()
        ...
        liableEarningsAmount.each(\value -> {dto.LiableEarningsAmount = new BigDecimal(value).toMonetaryAmount()})
    ...
    private function verifyPresenceOfMandatoryFields(
        ...
        liableEarningsAmount : Optional<String>) : List<FieldValidationError> {
      ...
      if (!liableEarningsAmount.isPresent())
        errors.add(new FieldValidationError("LiableEarningsAmount is required"))
      return errors
    }
  }

  /*--- ER Upload Processors ---*/
  class BulkERTransferPolicyProcessor extends AbstractCSVProcessor<ERTransferPolicyUploadDTO> {
    construct(rowParser : IRowParser<ERTransferPolicyUploadDTO>, updater : BulkUploadProcessUpdater, uploadFile : File) {
      super(rowParser, updater, uploadFile)
      _log = StructuredLogger.CONFIG.withClass(BulkERTransferPolicyProcessor)
    }

    override function processRows(rows : List<ERTransferPolicyUploadDTO>) : CSVProcessorResult {
      var dataRows = rows
      var rowProcessErrors = new ArrayList<RowProcessError>()
      _log.info("Importing ${dataRows.Count} ERTransferPolicys...")

      for (dataRow in dataRows index i) {
        var rowNumber = i + 1
        var lineNumber = i + 2
        createERTransferPolicy(lineNumber, dataRow, rowProcessErrors)
      }
      return new CSVProcessorResult(dataRows.Count - rowProcessErrors.Count, rowProcessErrors)
    }

    private function createERTransferPolicy(lineNumber : int, dto : ERTransferPolicyUploadDTO, rowProcessErrors : List<RowProcessError>) {
      try {
        var oEntity : ERTransferPolicy_ACC
        gw.transaction.Transaction.runWithNewBundle(\bundle -> {
          oEntity = new ERTransferPolicy_ACC()
          oEntity.PublicID = "er:"+dto.ERID

          if(dto.TransferID != null) {
            var oERTransferBuyer = getERTransferBuyer("er:" + dto.TransferID, dto.ACCPolicyID)
            if(oERTransferBuyer != null)
              oEntity.ERTransferBuyer = oERTransferBuyer
          }
          oEntity.LevyYear = dto.LevyYear
          oEntity.CUCode = dto.CUCode
          oEntity.LiableEarningsAmount = dto.LiableEarningsAmount
          _log.info("${lineNumber}: Created ER Transfer Policy")
        }, "sys")
        onSuccess()
      } catch (e : Exception) {
        _log.info("${lineNumber}: Creation of ER Transfer Policy failed for ${dto}: ${e.Message}")
        rowProcessErrors.add(new RowProcessError(lineNumber, e.Message))
        onFailure()
      }
    }

    private function getERTransferBuyer(publicID : String, accPolicyID : String) : ERTransferBuyer_ACC {
      var transfer = Query.make(ERTransfer_ACC)
      ...
      return Query.make(ERTransferBuyer_ACC)
          ...
          .compare(ERTransferBuyer_ACC#ACCPolicyID_ACC, Relop.Equals, accPolicyID)
          .select().AtMostOneRow
    }
  }
}