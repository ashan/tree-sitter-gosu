...
uses entity.windowed.CWPSEarnersLevyCostVersionList
...
class CWPSEarnersLevyCostData extends SHCCostData<CWPSEarnersLevyCost> {
  ...
  construct(cost: CWPSEarnersLevyCost) {
    super(cost)
    _coveredItemID = cost.CWPSCov.FixedId
  }

  construct(cost: CWPSEarnersLevyCost, rateCache: PolicyPeriodFXRateCache) {
    super(cost, rateCache)
    _coveredItemID = cost.CWPSCov.FixedId
  }

  override function setSpecificFieldsOnCost(line: CWPSLine, costEntity: CWPSEarnersLevyCost): void {
  ...
  override function getVersionedCosts(line: CWPSLine): List<EffDatedVersionList> {
    ...
    return coveredItemVL.CWPSEarnersLevyCosts.where(\costVL -> isCostVersionListForThisCostData(costVL))
  }

  private function isCostVersionListForThisCostData(costVL: CWPSEarnersLevyCostVersionList): boolean {
