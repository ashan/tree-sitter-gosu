...
class AccountPopulator {
  ...
  var _validUntil : Date
  var _irdNumber : String
  var _aepContractAccount_ACC : Boolean
  ...
  construct(searchCriteria : gw.account.AccountSearchCriteria) {
    ...
    _irdNumber = searchCriteria.IRDNumber_ACC
    _aepContractAccount_ACC = searchCriteria.AEPContractAccount_ACC
  }
  ...
  function createNewAccount(bundle : Bundle, contactType : ContactType) : Account {
    ...
    account.IRDNumber_ACC = this._irdNumber
    account.AEPContractAccount_ACC = _aepContractAccount_ACC
    return account
  }
  ...
  private function populateContact(contact : Contact) {
    ...
    address.ValidUntil = _validUntil
    address.Country = _country == null
                      ? PLDependenciesGateway.getCommonDependencies().getDefaultCountry()
                      : _country
    poplulatePolicyAddresses(address, contact)
  }

  private function poplulatePolicyAddresses(address : Address, contact : Contact) {
    if (contact typeis Person) {
