...
enhancement ContactEnhancement : entity.Contact {
  ...
  /**
   * Creates a WPS address
   * @param address to set as a WPS address
   */
  function makeWPSAddress_ACC(address : Address) {
    this.WPSAddress_ACC = address
  }
  ...
  /**
   * Creates a WPC address
   * @param address to set as a WPC address
   */
  function makeWPCAddress_ACC(address : Address) {
    this.WPCAddress_ACC = address
  }

  /**
   * Creates a CP / CPX address
   * @param address to set as a CP / CPX address
   */
  function makeCPCPXAddress_ACC(address : Address) {
    this.CPCPXAddress_ACC = address
  }

  /**
   * Returns the latest bound (submissions), completed or revised (audit periods)
   * that the contact is associated with.
   * Note: Results are filtered based on currently logged in user as part of original requirement
   * @return
   */
  public function findLatestBoundPolicyPeriods_ACC() : List<PolicyPeriod> {
    if (this.unknownShareholderDisabled_ACC()) {
      return {}
    }
    ...
    policyContactRoleTable.compare("ContactDenorm", Equals, this)
    ...
    pniContactDenormQuery.compare("PNIContactDenorm", Equals, this)
  ...
  /**
   * DE2283 - This method is used to disable the functionality of the unknown shareholder.
   * When Integration set the DummyContact_ACC to true in a later phase the unknown shareholder will be enabled.
   * @return
   */
  public function unknownShareholderDisabled_ACC() : boolean {
    var unknownShareholderACCID = ScriptParameters.getParameterValue("DummyContactACCID_ACC") as String
    return this.ACCID_ACC == unknownShareholderACCID and !this.DummyContact_ACC
  }

  /**
   * DE2283 - Returns true if the contact is an unknown shareholder i.e the ACC ID is A0000000.
   * @return
   */
  property get UnknownShareholder_ACC() : boolean {
    var unknownShareholderACCID = ScriptParameters.getParameterValue("DummyContactACCID_ACC") as String
    return this.ACCID_ACC == unknownShareholderACCID
  }

  property get HeldAccount() : Account {
    return this.AccountContacts.where(\elt -> elt.Account.AccountHolderContact == this).first().Account
  }

  public function isNamedInsuredContact_ACC() : boolean {
    var query = Query.make(AccountContactRole)
    query.compare(AccountContactRole#Subtype, Relop.Equals, typekey.AccountContactRole.TC_NAMEDINSURED)
  ...
  property get IRPhoneVerificationDetails_ACC() : String {
    var details = ""
    if (this.IRPhoneVerifiedStatus_ACC == null) {
      return details
    } else if (this.IRPhoneVerifiedStatus_ACC) {
      details = DisplayKey.get("Web.AccountFile.Summary.VerificationStatus.Verified_ACC")
    } else {
      details = DisplayKey.get("Web.AccountFile.Summary.VerificationStatus.Unverified_ACC")
    }

    if (this.IRPhoneValidUntil_ACC != null) {
      if (not details.Empty) {
        details = details.concat(" - ")
      }
      details = details.concat(DisplayKey.get("Web.AccountFile.Summary.VerificationStatus.ValidUntil_ACC", DateUtil_ACC.createDateAsString(this.IRPhoneValidUntil_ACC)))
    }
    return details
  }

  property get IREmailVerificationDetails_ACC() : String {
    var details = ""
    if (this.IREmailVerifiedStatus_ACC == null) {
      return details
    } else if (this.IREmailVerifiedStatus_ACC) {
      details = DisplayKey.get("Web.AccountFile.Summary.VerificationStatus.Verified_ACC")
    } else {
      details = DisplayKey.get("Web.AccountFile.Summary.VerificationStatus.Unverified_ACC")
    }

    if (this.IREmailValidUntil_ACC != null) {
      if (not details.Empty) {
        details = details.concat(" - ")
      }
      details = details.concat(DisplayKey.get("Web.AccountFile.Summary.VerificationStatus.ValidUntil_ACC", DateUtil_ACC.createDateAsString(this.IREmailValidUntil_ACC)))
    }
    return details
  }

  property get EmailVerificationDetails_ACC() : String {
    if (this.EmailAddress1 == null) {
      return null
    }
    if (this.EmailVerifiedDate_ACC != null) {
      return DisplayKey.get("Web.AccountFile.Summary.VerificationStatus.VerifiedOn_Acc", DateUtil_ACC.createDateAsString(this.EmailVerifiedDate_ACC))
    } else {
      return DisplayKey.get("Web.AccountFile.Summary.VerificationStatus.Unverified_ACC")
    }
  }
